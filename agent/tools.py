...
API_URL = os.getenv("API_URL", "https://simplify-money-assignment.onrender.com")
...
def get_gold_rate() -> str:
    ...
    try:
        ...
        return f"Error fetching gold rate: {str(e)}"
...
def buy_gold(username: str, email: str, amount: float, risk_level: str) -> str:
    ...
    try:
        ...
        return f"Error processing gold purchase: {str(e)}"
...
def get_gold_holdings(email: str) -> str:
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            return f"Your gold holdings are {data['gold_holdings_grams']} grams. Your current gold rate is Rs. {data['current_gold_rate_per_100g']} per 100 grams. Your total investment is Rs. {data['investment_amount']}. Your risk level is {data['risk_level']}."
        else:
            return f"Error: {response.status_code} - {response.text}"
    except Exception as e:
        return f"Error fetching gold holdings: {str(e)}"
...
def sell_gold(email: str, weight_to_sell: float) -> str:
    ...
    try:
        ...
        return f"Error processing gold sale: {str(e)}"
...
def our_agent(state: AgentState) -> AgentState:
    ...
    You are a helpful financial advisor answer the users queries to the best of your abilities. 
    ...
    if not state['messages']:
        ...
        # print(f"\n{Fore.BLUE}USER: {Style.RESET_ALL}{user_input}")
        user_message = HumanMessage(content=user_input)
    ...
    print(f"\n{Fore.GREEN}AI: {Style.RESET_ALL}{response.content}")
    if hasattr(response, "tool_calls") and response.tool_calls:
        print(f"{Fore.YELLOW}USING TOOLS: {Style.RESET_ALL}{[tc['name'] for tc in response.tool_calls]}")
...
def print_messages(messages):
    ...
    for message in messages[-3:]:
        if isinstance(message, ToolMessage):
            print(f"\n{Fore.MAGENTA}TOOL RESULT: {Style.RESET_ALL}{message.content}")
